###Setting the directory for paper 2 Impact of LLL (Final code file)
setwd("C:/Users/ZEF/Desktop/PhD/PhD paper/Paper 2 Impact/ML analysis")
##Uploading stata data to r
#install.packages('haven')
library(haven)
data <- read_dta('LLL Punjab plot data_revised2_paper1_adoption3_irri.dta')
...
freq_plot=bar_chart(data,treat_lll_new)+labs(y="Frequency of adoption", x= "Number of observation") + 
  scale_y_discrete(labels = c("2020-21", "Last 3 year", "Never","Before last 3 years" ))
freq_plot


###Plot showing the irrigation by frequency of adoption
library(ggpubr) 
library(tidyverse)
...
frequency_outcome_errorplot=
  ...
  ggerrorplot(x = "treat_lll_new", y = "irrigation_hrs_rice_ha",add = "mean", error.plot = "errorbar", color="black",size=1, ggtheme=theme_bw(), stat = "summary", fun.data = "mean_cl_boot")+
  labs(x="Frequency of adoption",y="Rice irrigation (h/ha)")+
...
factor_varibales <-c("levelled_2021", "caste_cat2",  "religion_new", "subsidy_lll",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob", "rice_short_duration", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "district")
...
library(policytree)
#data <- read_dta('LLL Punjab plot data_revised2_paper1_adoption3_irri.dta')
#data_freq=subset(data, select=c("kharif_rice_yld_ha","treat_lll","caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq"))
...
Y_cf_freq=as.vector(data$irrigation_hrs_rice_ha)
#Y_cf_freq =na.omit(Y_cf_freq)
has_na <- anyNA(Y_cf_freq)
print(has_na)

# Fit regression forest yield gains
X_rf_freq=subset(data, select=c("treat_lll","caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob", "rice_short_duration", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq", "district"))
X_rf_freq2=subset(data, select=c("treat_lll","caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob", "rice_short_duration", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq", "district"))
#X_rf_freq=na.omit(X_rf_freq)
#has_na <- anyNA(X_rf_freq)
#has_na
...
X_rf_freq.t1$treat_lll=0

X_rf_freq.t2=X_rf_freq
X_rf_freq.t2$treat_lll=1

X_rf_freq.t3=X_rf_freq
X_rf_freq.t3$treat_lll=2

X_rf_freq.t4=X_rf_freq
X_rf_freq.t4$treat_lll=3
...
ggplot(rf_pred_contrasts_long, aes(x=value, y=variable, fill = factor(stat(quantile)))) +
  ...
  theme_bw(base_size = 16)+labs(x="Irrigation(hrs/ha)",y="Frequency of laser land levelling")
...
data=subset(data, select=c("irrigation_hrs_rice_ha", "levelled_2021", "treat_lll_new", "caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob","rice_short_duration", "pusa_44", "pump_hp_new", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq", "district"))
library(tidyr)
data=data%>% drop_na()
Y_cf_freq=as.vector(data$irrigation_hrs_rice_ha)
...
vars_freq=c("caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob","rice_short_duration","pusa_44", "pump_hp_new", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq", "district")
...
multi_freq.forest_binary <- causal_forest(X = X_cf_freq, Y = Y_cf_freq, W = W_cf_freq_binary ,W.hat=W.hat.multi.all_freq_binary,Y.hat=Y.hat.multi.all_freq_binary,seed=2) 
...
#RATE
tau.hat.cf_freq <-predict(multi_freq.forest_binary,X_cf_freq )$predictions
