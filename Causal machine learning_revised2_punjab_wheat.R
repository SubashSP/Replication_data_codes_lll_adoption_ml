###Setting the directory for paper 2 Impact of LLL
setwd("C:/Users/ZEF/Desktop/PhD/PhD paper/Paper 2 Impact/ML analysis")
##Uploading stata data to r
#install.packages('haven')
library(haven)
data <- read_dta('LLL Punjab plot data_revised2_paper1_adoption3_wheat_yield2.dta')
table(data$treat_lll_new) 
#table(data$rice_short_duration)
# Bar graphs showing frequency of adoption of LLL in the sample
library(tidyverse) 
library(ggplot2) 
...
freq_plot=bar_chart(data,treat_lll)+labs(y="Frequency of adoption", x= "Number of observation") + 
  scale_y_discrete(labels = c("2020-21", "Last 3 year", "Never","Before last 3 years" ))
freq_plot


###Plot showing the wheat yield by frequency of adoption
library(ggpubr) 
library(tidyverse)
...
frequency_outcome_errorplot=
  ...
  ggerrorplot(x = "treat_lll_new", y = "rabi_wheat_yld_ha",add = "mean", error.plot = "errorbar", color="black",size=1, ggtheme=theme_bw(), stat = "summary", fun.data = "mean_cl_boot")+
  labs(x="Frequency of adoption",y="Wheat yield (kg/ha)")+
  ##geom_text(aes(label = round(..y.., digits = 2), y = ..y..), vjust = -0.5, color = "blue", size = 3, stat = "summary", fun.y = mean)
  theme_bw(base_size = 16)+coord_flip()

frequency_outcome_errorplot

frequency_outcome_errorplot2 = frequency_outcome_errorplot+ aes(x = fct_reorder(treat_lll, kharif_rice_yld_ha))+
  xlab("Frequency of adoption")
frequency_outcome_errorplot2
...
data_desc=fastDummies::dummy_cols(data, select_columns=c("levelled_2021", "caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "rice_short_duration", "plot_waterloggprob", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "district"))

ls(data_desc)
library(fBasics)
summ_stats <- fBasics::basicStats(data_desc[,c("age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq", "rice_duration_days", "_Iaccess_fa_1", "_Iaccess_go_1","_Iaccess_in_1", "_Iaccess_kv_1" , "_Iaccess_ng_1", "_Iaccess_pr_1", "_Icaste_cat_2", "_Iplot_soil_2", "_Iplot_wate_1", "rice_short_duration_3", "_Ireligion__1" , "district_1", "district_2", "district_3", "district_4")])
...
factor_varibales <-c("levelled_2021", "caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob", "rice_short_duration", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "district")
...
# FGLS
ls(data)
ols = lm(rabi_wheat_yld_ha ~treat_lll_order+age_hhmember+ edun_hhmember+caste_cat2+religion_new+total_num_adult_members_HH+women_share+non_farm_binary+asset_index+ plot_area_ha+ num_plot+plot_soiltype2+plot_soilfertility+plot_erosionprobl+plot_waterloggprob+sp_lll_new+access_govt_ext+access_kvk+access_prog_farmer+access_ngo+access_farmer_grp+access_input_dealer+ datairrwater_tbl_21+simpson_index_kh+simpson_index_rb+ datadstdst_dhq+district, data = data)
summary(ols)
ols2 = lm(rabi_wheat_yld_ha ~treat_lll_order+age_hhmember+ edun_hhmember+caste_cat2+religion_new+total_num_adult_members_HH+women_share+non_farm_binary+asset_index+ plot_area_ha+ num_plot+plot_soiltype2+plot_soilfertility+plot_erosionprobl+plot_waterloggprob+sp_lll_new+access_govt_ext+access_kvk+access_prog_farmer+access_ngo+access_farmer_grp+access_input_dealer+ datairrwater_tbl_21+simpson_index_kh+simpson_index_rb+ datadstdst_dhq, data = data)
summary(ols2)
...
#Y_cf_freq =na.omit(Y_cf_freq)
has_na <- anyNA(Y_cf_freq)
print(has_na)

# Fit regression forest yield gains
X_rf_freq=subset(data, select=c("treat_lll_new","caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob",  "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq", "district"))
X_rf_freq2=subset(data, select=c("treat_lll_new","caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq"))
X_rf_freq2=na.omit(X_rf_freq2)
has_na <- anyNA(X_rf_freq2)
has_na
#X_rf_freq = X_rf_freq %>% drop_na()
Y.multi_freq.forest.rf <- regression_forest(X_rf_freq2, Y_cf_freq,
...
X_rf_freq.t1=X_rf_freq2
X_rf_freq.t1$treat_lll_new=0

X_rf_freq.t2=X_rf_freq2
X_rf_freq.t2$treat_lll_new=1

X_rf_freq.t3=X_rf_freq2
X_rf_freq.t3$treat_lll_new=2

X_rf_freq.t4=X_rf_freq2
X_rf_freq.t4$treat_lll_new=3

# Calculate irrigation differences against frequency of laser land levelling
...
ggplot(rf_pred_contrasts_long, aes(x=value, y=variable, fill = factor(stat(quantile)))) +
  ...
  theme_bw(base_size = 16)+labs(x="Wheat yield(kg/ha)",y="Frequency of laser land levelling")

###Causal random forest model

#X_cf_freq=subset(data, select=c("caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob","rice_short_duration", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq", "district"))
X_cf_freq2=subset(data, select=c("caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob", "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "num_plot", "simpson_index_kh", "simpson_index_rb", "datadstdst_dhq"))

W_cf_freq <- as.factor(data$treat_lll_new)

##Probability random forest to create weights
W.multi_freq.forest <- probability_forest(X_cf_freq2, W_cf_freq,
                                          equalize.cluster.weights = FALSE,
                                          seed = 2
)
...
Y.multi_freq.forest <- regression_forest(X_cf_freq2, Y_cf_freq,
                                         equalize.cluster.weights = FALSE,
                                         seed = 2
)
...
multi_freq.forest <- multi_arm_causal_forest(X = X_cf_freq2, Y = Y_cf_freq, W = W_cf_freq ,W.hat=W.hat.multi.all_freq,Y.hat=Y.hat.multi.all_freq,seed=2) 
...
vars_freq=c("treat_lll_new","caste_cat2",  "religion_new",  "non_farm_binary", "plot_soiltype2", "plot_soilfertility", "plot_erosionprobl", "plot_waterloggprob",  "access_govt_ext" , "access_kvk", "access_prog_farmer", "access_ngo", "access_farmer_grp", "access_input_dealer", "age_hhmember", "edun_hhmember", "total_num_adult_members_HH", "women_share", "asset_index", "plot_area_ha", "plot_dist_lll", "num_plot", "sp_lll_new", "datairrwater_tbl_21","simpson_index_kh", "simpson_index_rb", "datadstdst_dhq")
...
# Probability random forest to create weights
W.multi_freq.forest_binary <- regression_forest(X_cf_freq2, W_cf_freq_binary,
                                                equalize.cluster.weights = FALSE,
                                                seed = 2
)
...
multi_freq.forest_binary <- causal_forest(X = X_cf_freq2, Y = Y_cf_freq, W = W_cf_freq_binary ,W.hat=W.hat.multi.all_freq_binary,Y.hat=Y.hat.multi.all_freq_binary,seed=2) 
...
# Fit binary causal RF model with adjusted propensity score
#Checking for distribution of common support
hist(W.hat.multi.all_freq_binary, xlab = "Propensity score")
sum(W.hat.multi.all_freq_binary< 0.1, na.rm=TRUE)
sum(W.hat.multi.all_freq_binary> 0.5, na.rm=TRUE)
##https://academic.oup.com/biomet/article/96/1/187/235329 

W.hat.multi.all_freq_binary[W.hat.multi.all_freq_binary<0.1]=NA
W.hat.multi.all_freq_binary[W.hat.multi.all_freq_binary>0.5]=NA
has_na <- anyNA(W.hat.multi.all_freq_binary)
print(has_na)
summary(has_na)
weight=na.omit(W.hat.multi.all_freq_binary)
...
multi_freq.forest_binary <- na.omit(multi_freq.forest_binary)
has_na <- anyNA(multi_freq.forest_binary)
has_na
multi_freq_binary_calibration=test_calibration(multi_freq.forest_binary)
multi_freq_binary_calibration
...
#RATE
tau.hat.cf_freq <-predict(multi_freq.forest_binary,X_cf_freq2 )$predictions
cf_freq_rate <-rank_average_treatment_effect(multi_freq.forest_binary, tau.hat.cf_freq , target = "AUTOC")

### Policy tree algorithm
## Compute robust scores
DR.scores_freq <- double_robust_scores(multi_freq.forest)
X_cf_freq2 = X_cf_freq2 %>% drop_na()
nrow(X_cf_freq)
nrow(DR.scores_freq)
##Matching the row missing
X_cf_freq <- X_cf_freq2[1:min(nrow(X_cf_freq), nrow(DR.scores_freq)), ]
DR.scores_freq <- DR.scores_freq[1:min(nrow(X_cf_freq), nrow(DR.scores_freq)), ]

#install.packages("DiagrammeR")
library(DiagrammeR)
tr_freq <- policy_tree(X_cf_freq2, DR.scores_freq, depth = 2) 
plot(tr_freq)
tr_freq3 <- hybrid_policy_tree(X_cf_freq2, DR.scores_freq, depth = 3) 
plot(tr_freq3)
...
tr_assignment_freq$treat_lll_cat[tr_assignment_freq$treat_lll == 0] <- "Never"
tr_assignment_freq$treat_lll_cat[tr_assignment_freq$treat_lll == 1] <- "Before 3 yrs"
tr_assignment_freq$treat_lll_cat[tr_assignment_freq$treat_lll == 2] <- "Last 3 yrs"
tr_assignment_freq$treat_lll_cat[tr_assignment_freq$treat_lll == 3] <- "2020-21"

library(ggalluvial)
library(data.table)
tr_assignment_freqDT = data.table(tr_assignment_freq)
TransitionMatrix_freq <- tr_assignment_freqDT[, (sum <- .N), by = c("treat_lll_cat", "depth2_cat")]
library(dplyr)
TransitionMatrix_freq <- rename(TransitionMatrix_freq, Freq = V1)
summary(TransitionMatrix_freq)
library(scales)
transitionmatrixplot_freq <- ggplot(
  ...
  scale_x_discrete(
    limits = c("treat_lll_cat", "depth2_cat"),
    expand = c(0.15, 0.05)
  ) +
